...
Scripts:
- `scripts/build_deb.sh` builds `.deb` artifacts for both client and server by invoking the new `client/build.sh` and `server/build.sh` PyInstaller steps, staging files under `/opt/playpalace/`, and running `dpkg-deb`.
- `scripts/build_rpm.sh` mirrors the process for `.rpm` packages using `rpmbuild`, tarballing the PyInstaller outputs and installing the systemd unit/desktop entry from `installer/linux/systemd/` and `installer/linux/client/`.
...
When we flesh the remaining formats out, place distro-specific tooling under `installer/linux/<family>/` (e.g., `debian/`, `rpm/`, `arch/`). Each package should:
